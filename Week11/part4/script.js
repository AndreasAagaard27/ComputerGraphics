...
var lastVec = null;
var lastIncrementalRotation = new Quaternion();
lastIncrementalRotation.setIdentity();
var spinning = false;
var lastMouseTime = 0;
...
function onMouseMove(event) {
    ...
    if (mouseButton === 0) {
        ...
        if (lastVec && currentVec) {
            ...
            lastVec = currentVec;
            lastIncrementalRotation.set(out.elements);
            spinning = true;
        }
...
function render() {
    if (!isMouseDown && spinning) {
        ...
        if (now - lastMouseTime > 20) {
            ...
            var tmp = new Quaternion(cameraRotation.elements);
            cameraRotation.set(lastIncrementalRotation);
            cameraRotation.multiply(tmp);
            normalizeQuaternion(cameraRotation);
        }
    }
